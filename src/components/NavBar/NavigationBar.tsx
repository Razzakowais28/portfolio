...
export default function NavigationBar() {
    ...
    return (
        <nav className="fixed w-full top-0 flex px-4 sm:px-6 md:px-16 h-16 bg-background text-foreground justify-between items-center border-b-4 border-foreground z-50 transition-colors duration-300 gap-2 sm:gap-4">
            {/* Logo + live status */}
            <div className="flex items-center gap-2 sm:gap-4 min-w-0">
                <div className="font-mono font-black text-2xl border-3 border-foreground bg-card text-foreground px-3 py-0.5 shadow-[3px_3px_0px_0px_var(--foreground)] tracking-tighter select-none shrink-0">
                    MO_
                </div>
                <LiveStatusBadge className="min-w-0 truncate" />
            </div>
            ...
            {isOpen && (
                <div className="absolute top-full left-0 w-full bg-background border-b-4 border-foreground flex flex-col items-stretch py-4 px-6 gap-2 z-10 transition-all duration-300 shadow-[0_4px_0px_0px_var(--foreground)] lg:hidden">
                    {navItems.map((item) => (
